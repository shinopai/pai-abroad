<nav
  class="relative w-full flex items-center justify-between py-4 text-gray-500 hover:text-gray-700 focus:text-gray-700"
>
  <div
    class="w-[15%] container-fluid flex items-center justify-between px-6"
  >
  ...
  <ul class="flex justify-end pr-6">
    ...
    <li><form action="{{ route('logout') }}" method="post" id="logout_form">
      @csrf
    </form>
    <a href="{{ route('logout') }}" id="logout_btn" class="inline-block" onclick="event.preventDefault(); logout();">ログアウト</a></li>
    @else
